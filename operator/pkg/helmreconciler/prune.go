...
func filterOutGateways(pids []string, name, namespace string) []string {
	result := make([]string, 0, len(pids))
	for _, pid := range pids {
		if strings.HasPrefix(pid, name+"-") && strings.HasSuffix(pid, "."+namespace) {
			continue
		}
...
func (h *HelmReconciler) PruneControlPlaneByRevisionWithController(iopSpec *v1alpha1.IstioOperatorSpec) (*v1alpha1.InstallStatus, error) {
	...
	if pilotEnabled {
		...
		if pilotExists {
			...
			// we can not get the act value from iopSpec.Components.IngressGateways when it is delete,the value is null
			...
			pids = filterOutGateways(pids, "istio-eastwestgateway", "istio-system")
